...
execute if score @s aj.run.frame matches 21.. run scoreboard players set @s aj.run.frame 1
data remove storage animated_java:temp args
execute store result storage animated_java:temp args.frame int 1 run scoreboard players get @s aj.run.frame
execute at @s run function animated_java:female/animations/run/zzz/apply_frame with storage animated_java:temp args
