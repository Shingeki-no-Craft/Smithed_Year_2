...
$execute if score @s[tag=!snc.wire.retract, tag=!snc.wire.shifter] snc.odm_dist matches 25.. as $(name) run function snc:player/odm/hook/shake
execute if score @s[tag=!snc.wire.retract, tag=!snc.wire.shifter] snc.odm_dist matches 25.. run tag @s add snc.wire.retract
execute if score @s[tag=!snc.wire.retract, tag=!snc.wire.shifter] snc.odm_dist matches 25.. run scoreboard players set @s snc.projectile 0
