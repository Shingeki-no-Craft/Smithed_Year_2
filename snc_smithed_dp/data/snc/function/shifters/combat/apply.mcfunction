...
$execute if entity @s[tag=transform] if predicate snc:chance/1 as @n[scores={$(shifter)_vars=1}] run function snc:logic/trigger/dialogue/titan/mastery {"shifter":"$(shifter)"}
...
$execute if entity @s[tag=transform] on vehicle positioned as @s rotated ~ -$(angle) run function snc:logic/motion/generic {"score":"$(shifter)_vars","strength":$(knockback), "unstoppable":"true"}
