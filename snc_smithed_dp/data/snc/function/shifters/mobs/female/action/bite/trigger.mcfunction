...
# Bite while Sneak
execute if score state female_vars matches 2 on vehicle positioned as @s rotated ~ 0 positioned ^ ^-.9 ^4 if entity @e[distance=..3,type=#snc:human,tag=!snc.titan,predicate=!snc:shifters/female/score] on passengers if entity @s[tag=animated_java.female.root,tag=!animated_java.female.animation.bite.playing] if function snc:shifters/mobs/female/animate/consume run return -1
# Bite standing
