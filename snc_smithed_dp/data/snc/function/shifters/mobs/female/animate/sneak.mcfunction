...
execute if score state female_vars matches 3..5 unless predicate snc:player/keybinds/move on vehicle on passengers \
        if entity @s[tag=aj.female.root] if entity @s[tag=!aj.female.animation.sneak.playing] run \
            scoreboard players set #sneak female_vars 1

execute if score #sneak female_vars matches 1 on vehicle on passengers if entity @s[tag=aj.female.root] run function animated_java:female/animations/pause_all
execute if score #sneak female_vars matches 1 on vehicle on passengers if entity @s[tag=aj.female.root] run function animated_java:female/animations/sneak/play
