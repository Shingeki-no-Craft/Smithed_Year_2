...
$execute if entity @s[tag=animated_java.$(shifter).locator.consume] if score $hold $(shifter)_vars matches 1 unless data entity @s Passengers run ride @n[type=#snc:human,tag=!transform,distance=..5] mount @s
...
$execute as @s[tag=snc.body] run function snc:shifters/mobs/vehicle with storage minecraft:$(shifter)
$execute as @s[tag=snc.collision] run function snc:shifters/mobs/collision {"shifter":"$(shifter)", "id":"$(id)"}

# If it transforms
$execute if entity @s[tag=animated_java.$(shifter).locator.consume] on passengers run ride @s[tag=transform] dismount
