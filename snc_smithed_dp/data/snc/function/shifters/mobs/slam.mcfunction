...
$execute if score $air_frame $(shifter)_vars matches 0..19 on vehicle anchored feet as @e[type=player, distance=..16,tag=!$(shifter)] run function snc:shifters/combat/slam {"distance":6, "damage":8}
...
$execute if score $air_frame $(shifter)_vars matches 20..39 on vehicle anchored feet as @e[type=player, distance=..16,tag=!$(shifter)] run function snc:shifters/combat/slam {"distance":10, "damage":16}
...
$execute if score $air_frame $(shifter)_vars matches 31.. on vehicle anchored feet as @e[type=player, distance=..16,tag=!$(shifter)] run function snc:shifters/combat/slam {"distance":12, "damage":32}
